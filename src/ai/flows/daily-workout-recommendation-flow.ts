 ...
 * flow that generates a `structuredWorkout` field (this one and
 * planWeekSessions) — a single source of truth for the precision-critical
 * format the site's own parser expects (see the "Nx (étape / étape)"
 * warning below: that inline shorthand looks plausible but silently
 * produces a workout with no steps).
 */
