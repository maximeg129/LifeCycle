...
export function AddWaxHistoryDialog({ chain, open, onOpenChange, entry = null }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    }, { path: chainRef.path, operation: isEditing ? 'update' : 'create' })
    ...
    if (ok) {
      if (isEditing) {
        ...
        form.reset() // Form stays open to log several rotations in a row.
      }
    }
  }
  ...
  return (
    <CrudDialogShell
      key={entry?.id ?? 'new'}
      title={<span className="flex items-center gap-2"><History className="w-4 h-4" /> {isEditing ? 'Modifier' : 'Historique de fartage'} — {chain.label}</span>}
      ...
      cancelLabel={isEditing ? 'Annuler' : 'Terminé'}
